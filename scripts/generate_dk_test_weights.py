...
def generate_weights(config: dict) -> dict[str, torch.Tensor]:
    ...
    # --- Embedding ---
    weights["model.embed_tokens.weight"] = r((vocab, h))
    ...
    for layer_idx in range(n_layers):
        ...
        if layer_idx + 1 in kda_layers:
            ...
            # === DeepSeek V4 layer ===
            # Attention (simplified — just the essential params)
            attn = pfx + "attn."
            # Q projections
            ...
            # Indexer
            weights[attn + "indexer.weights_proj.weight"] = r((1, n_heads * head_dim))
            # Compressor
    ...
    weights["lm_head.weight"] = r((vocab, h))
