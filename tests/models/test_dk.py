...
from vllm.model_executor.models.dk import ProjectionIn, ProjectionOut
...
def test_generate_weights():
    """Test that the weight generation script produces a valid checkpoint."""
    ...
    # Verify key parameters exist (HF naming convention)
    assert "embed.weight" in weights, "Missing embedding"
    assert "head.weight" in weights, "Missing LM head"
    assert "model.norm.weight" in weights, "Missing final norm"

    # Verify KDA layers have projections (note: .layer. prefix from DKDecoderLayer)
    assert "model.layers.1.layer.proj_in.proj.weight" in weights, \
        "Missing KDA proj_in at layer 2"
    assert "model.layers.1.layer.proj_out.proj.weight" in weights, \
        "Missing KDA proj_out at layer 2"

    # Verify DeepSeek layers have MHC params
    assert "model.layers.0.layer.hc_attn_fn" in weights, "Missing MHC at layer 1"
    assert "model.layers.0.layer.hc_ffn_fn" in weights, "Missing MHC at layer 1"
...
def test_tiny_model_inference():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Use raw token IDs since we skip tokenizer init (no tokenizer in test checkpoint)
        prompt_token_ids = [1, 2, 3, 4, 5]  # arbitrary valid token IDs
        ...
        print(f"Generated {len(outputs[0].outputs[0].token_ids)} tokens: "
              f"{outputs[0].outputs[0].token_ids}")
